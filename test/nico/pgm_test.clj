...
(with-test
  (defn fit? [n]
    (let [npgms (count-pgms)
      ...
      (= n npgms ncomm npubdate nupdated nelapsed)))
  (defn apply-single [f n]
    (doseq [pgm (create-pgms 0 5000)] (f pgm)))
  (defn rm [pgm] (@#'nico.pgm/rem-aux (:id pgm)))
  ...
  (testing "add with constraint"
    (set-total 3000)
    (is (do (apply-single add 5000) (fit? 3150)) "add 5000 programs multi with max 3000")))
;;    (is (do (apply-multi add 5000 4) (fit? 3150)) "add 5000 programs multi with max 3000")))
