...
(ns #^{:author "sgr"
  ...
  (:import [java.awt BorderLayout Dimension Window]
           [javax.swing BorderFactory GroupLayout SpringLayout JComponent
            JButton JDialog JLabel JPanel JPasswordField JTextField]
           [javax.swing.event DocumentListener]
           [javax.swing.text PlainDocument]))

(def ^{:private true} ^Dimension DLG-SIZE (Dimension. 450 150))
(def ^{:private true} ^Dimension BTN-PANEL-SIZE (Dimension. 450 40))

(defn- kv-dlg
  [^Window parent ^String title ^String label-key key ^String label-val val ok-fn secure]
  (let [dlg (JDialog. parent title true)
        cpane (.getContentPane dlg), tkey (JTextField. 25),
        ^JTextField tval (if secure (JPasswordField. 25) (JTextField. 25))
        tkey-border (.getBorder tkey), tval-border (.getBorder ^JComponent tval)
...
(defn ^JDialog user-password-dialog
...
(defn ^JDialog browser-command-dialog
