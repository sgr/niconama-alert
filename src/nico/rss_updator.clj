...
(let [counter (atom 1)
  ...
  (defn- fetch-rss []
    (try
      (loop [page 1, total-count 0, pids #{}, no-pgms 0]
        (let [[cur-total cur-pgms] (rss/get-programs-from-rss page)
              total (max total-count ^long cur-total)
              pids-updated (reduce conj pids (map :id cur-pgms))
              cids (count pids-updated)]
          (trace (format "fetched RSS(%d) cur-total: %d" page total))
          (when (< (pgm/get-total) total) (pgm/set-total total))
          (trace (format "adding fetched pgms of RSS(%d)" page))
          ;; 番組の追加と取得状況のリアルタイム更新
          (pgm/add-pgms cur-pgms)
          ...
          (cond
           ...
           (l/with-info (format "finished fetching programs: %d" cids)
             (pgm/set-total total)
             [:finished cids total])
           (= 0 (count cur-pgms)) ;; ひとつも番組が取れない場合は中止
  ...
  (defn update-rss []
    (try
      (loop [max @counter]
        (if (= 1 (.getCount ^CountDownLatch @latch)) ;; pause中かどうか
          (do (.await ^CountDownLatch @latch))
  ...
  (defn pause-rss-updator []
    (if (= 1 (.getCount ^CountDownLatch @latch))
      (do (.countDown ^CountDownLatch @latch) false)
      (do (reset! latch (CountDownLatch. 1)) true))))
