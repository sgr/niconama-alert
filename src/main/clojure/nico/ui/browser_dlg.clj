...
(ns #^{:author "sgr"
  ...
  (:import [java.awt BorderLayout Dimension Frame]
           ...
           [javax.swing.table DefaultTableCellRenderer DefaultTableModel TableColumn
            TableCellEditor TableCellRenderer TableColumnModel TableModel]))

(def ^{:private true} ^Dimension DLG-SIZE (Dimension. 500 270))
(def ^{:private true} ^Dimension BTN-PANEL-SIZE (Dimension. 500 40))

(defn- browsers-to-model [browsers]
  (let [model (proxy [DefaultTableModel][]
                (isCellEditable [r c]
                  (if (= 2 c)
                    (if (.getValueAt ^DefaultTableModel this r c) false true)
                    false)))]
...
(defn- model-to-browsers [^DefaultTableModel model]
...
(defn- string-renderer []
  (proxy [DefaultTableCellRenderer] []
    (getTableCellRendererComponent
     [^JTable tbl val isSelected hasFocus row col]
     (let [^JLabel c (proxy-super getTableCellRendererComponent tbl val isSelected hasFocus row col)]
       (condp = col
...
(defn- set-cols! [^JTable tbl cr]
  (let [^TableColumnModel cm (.getColumnModel tbl), cb (JCheckBox.), bce (DefaultCellEditor. cb)]
    (doto bce
...
(defn- ^JTable browser-tbl [^TableModel model]
  (letfn [(checked-row [^TableModel m] (some #(if (.getValueAt m % 2) % false) (range (.getRowCount m))))]
    (let [cr (atom -1)
          tbl (proxy [JTable][]
                (editCellAt [r c e] (let [result (proxy-super editCellAt r c e)]
                                      (when result (reset! cr (checked-row (.getModel ^JTable this))))
                                      result))
                (setModel [^TableModel m] (proxy-super setModel m) (set-cols! this cr)))]
      (doto tbl (.setModel model)))))

(defn ^JDialog browsers-dialog
  [^Frame parent ^String title browsers ok-fn]
  (let [dlg (JDialog. parent title true), cpane (.getContentPane dlg)
    ...
    (let [tbl-sel-model (DefaultListSelectionModel.)
        ...
        (add-action-listener
         (fn [e]
           (let [bcd (ukvd/browser-command-dialog
                      dlg "ブラウザの追加" nil nil
                      (fn [name cmd] (.addRow ^DefaultTableModel (.getModel tbl) (to-array [name cmd false]))))]
      ...
      (doto tbtn-rem
        (add-action-listener
         (fn [e] (.removeRow ^DefaultTableModel (.getModel tbl) (.getSelectedRow tbl)))))
      (doto tbtn-up
        (add-action-listener
         (fn [e] (let [r (.getSelectedRow tbl), to-r (dec r)]
                   (when-not (= 0 r)
                     (.moveRow ^DefaultTableModel (.getModel tbl) r r to-r)
                     (.setSelectionInterval tbl-sel-model to-r to-r))))))
      (doto tbtn-down
        (add-action-listener
         (fn [e] (let [r (.getSelectedRow tbl), to-r (inc r)
                       ^DefaultTableModel m (.getModel tbl)
                       maxr (dec (.getRowCount m))]
                   (when-not (= maxr r)
                     (.moveRow m r r to-r)
                     (.setSelectionInterval tbl-sel-model to-r to-r))))))
      (doto hgrp
